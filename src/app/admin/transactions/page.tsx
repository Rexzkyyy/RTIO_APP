    ...
    return (
      <div className="w-full space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {events.map(event => (
            <Link key={event.id} href={`/admin/transactions?eventId=${event.id}&status=PENDING`} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-emerald-500 hover:shadow-md transition-all group">
  ...
  if (isValidator && eventId && !allowedEventIds.includes(eventId)) {
    return (
      <div className="w-full space-y-6">
        ...
        <Link href="/admin/transactions" className="inline-flex items-center px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 font-medium">
    ...
    <div className="w-full space-y-6">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 pt-4">
          {page > 1 ? (
            <Link href={`/admin/transactions?eventId=${eventId}&q=${encodeURIComponent(q)}&status=${statusParam}&page=${page - 1}`} className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 shadow-sm">
