...
export async function createEvent(formData: FormData) {
  ...
  const ticketCategories = ticketNames.map((name, idx) => {
    ...
    return {
      ...
      discountStartDate: discountStartDates[idx] ? new Date(`${discountStartDates[idx]}+08:00`) : null,
      discountEndDate: discountEndDates[idx] ? new Date(`${discountEndDates[idx]}+08:00`) : null,
  ...
  await prisma.event.create({
    data: {
      ...
      slug,
      description,
      eventDate: new Date(`${eventDateStr}+08:00`),
      location,
      artists,
      sponsors,
