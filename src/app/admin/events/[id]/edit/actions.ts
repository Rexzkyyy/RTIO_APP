...
export async function updateEvent(id: string, formData: FormData) {
  ...
  for (let i = 0; i < ticketIds.length; i++) {
    ...
    const baseData: any = {
      ...
      discountStartDate: discountStartDates[i] ? new Date(`${discountStartDates[i]}+08:00`) : null,
      discountEndDate: discountEndDates[i] ? new Date(`${discountEndDates[i]}+08:00`) : null,
