...
export default function WhatsAppSaveButton({ eventTitle, transactionId, buyerName, isPending }: { eventTitle: string, transactionId: string, buyerName?: string, isPending: boolean }) {
  ...
  return (
    <button 
      onClick={handleShare}
      className={`w-full flex justify-center items-center px-6 py-4 bg-[#25D366] text-white font-bold rounded-xl hover:bg-[#128C7E] transition-all shadow-lg shadow-[#25D366]/30 ${isPending ? 'mt-4' : ''}`}
    >
      ...
      {isPending ? 'Simpan link ke wa saya' : 'Kirim Tautan ke WA Sendiri'}
    </button>
  );
}
